...
JSON_PROD = os.environ.get('JSON_PROD')
JSON_TEST = os.environ.get('JSON_TEST')
SAVE_PATH = os.environ.get('SAVE_PATH')
LOCAL_URL = os.environ.get("LOCAL_POSTGRE_URL")
...
async def async_bs4_template(pipeline):
						...
						if strategy == "main":
							...
							try:
								if container:
									...
									for title_element, link_element, description_element in job_elements:
							...
							# Find all h2 tags
							titles = soup.find_all(elements_path["title_path"])
							links = soup.find_all(elements_path["link_path"])
							descriptions = soup.find_all(elements_path["description_path"])
	...
	async def gather_json_loads_bs4(session):
		...
		if title_len == link_len == description_len == timestamp_len:
